...
int main(){
	...
	VkSemaphore *waitSemaphores = createSemaphores(&device, maxFrames), 
							*signalSemaphores = createSemaphores(&device, maxFrames);
	VkFence *frontFences = createFences(&device, maxFrames), 
					*backFences = createEmptyFences(swapchainImageNumber);
